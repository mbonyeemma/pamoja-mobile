...
export default ({data, styles}) => {
  return (
    <ScrollView horizontal style={[{width: '100%', paddingRight: 5,}, styles ]}>
      {dataS.map(item => (
      <View key={Math.random()} style={{alignSelf: 'center', borderRadius: 10, marginLeft: 15, justifyContent: 'center', alignItems: 'center', width: 60, height: '100%'}}>
        ...
        <Text allowFontScaling style={{marginTop: 5, color: '#aaa', fontSize: 9}}>{item.title}</Text> 
